            ...
            async {
                Ok(req.into_response(
                    HttpResponse::Gone()
                        ...
                        .body(r#"{"error":"api_deprecated","description":"You are using an application that uses an outdated version of Modrinth's API. Please either update it or switch to another application. For developers: https://docs.cosmods.miki85.cz/api/#versioning"}"#)
                ))
            }.boxed_local()
        })
    );
