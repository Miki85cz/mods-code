...
[Go browse our plugin section!](https://cosmods.miki85.cz/plugins)
...
Modrinth is taking a unique approach to this. While the search pages are separate, in reality, the backend is the same. You can select plugin loaders when creating a mod and you can select mod loaders when creating a plugin. The split only exists on the frontend so that projects like [Chunky](https://cosmods.miki85.cz/mod/chunky) can share a single page across their versions.
...
[Go browse our resource pack section!](https://cosmods.miki85.cz/resourcepacks)
...
Resource packs are able to set dependencies on other projects (even those which aren't resource packs), just like how modpacks are able to set dependencies on mods. It's worth noting that OptiFine is not on the platform, and thus you cannot set a dependency on that; however, you can set a dependency on any of the other alternative mods which _are_ available on Modrinth, including [Entity Texture Features](https://cosmods.miki85.cz/mod/entitytexturefeatures), [OptiGUI](https://cosmods.miki85.cz/mod/optigui), [Continuity](https://cosmods.miki85.cz/mod/continuity), [CIT Resewn](https://cosmods.miki85.cz/mod/cit-resewn), [Animatica](https://cosmods.miki85.cz/mod/animatica), or [Custom Entity Models](https://cosmods.miki85.cz/mod/cem).
...
The changes in this update are rather minimal when it comes to API-related stuff. Two new fields have been added to the [project struct](https://docs.cosmods.miki85.cz/api-spec/#tag/project_model) - `approved`, which is the timestamp of when the project was approved (null if it's not approved or unlisted), and `additional_categories`, another set of categories which are to be seen as less important than normal categories. You can read the [secondary categories](#secondary-categories) section for more info on it. If you wish to implement the headers in your API integration, the [category list](https://docs.cosmods.miki85.cz/api-spec/#tag/tags/operation/categoryList) now has a `header` field.

As for the [search result struct](https://docs.cosmods.miki85.cz/api-spec/#tag/project_result_model), `created` now matches the `approved` date rather than the `published` project field, and `categories` now also includes secondary categories. A new field, `display_categories`, matches only primary categories.
...
[Modrinth Maven]: https://support.cosmods.miki85.cz/en/articles/8801191-modrinth-maven
